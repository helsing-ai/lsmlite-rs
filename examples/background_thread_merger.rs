// You can execute this example with `cargo run --example background_thread_merger`
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // must increase as data is persisted (so it's made durable). Metrics would
    // be another way to check that a background thread is working.
